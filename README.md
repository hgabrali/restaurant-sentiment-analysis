...
3. **Mean word count by sentiment:** negative ≈ 24 | neutral ≈ 26 | positive ≈ 24
...
5. **Top negative terms (TF-IDF):** `experience`, `wrong`, `almost`, `returning`, `certainly`
6. **Word-count vs rating correlation:** r = −0.170 (weak negative — longer reviews lean slightly negative)
7. **Rating by cuisine:** best = Italian (avg 3.57 ★) | worst = Indian (avg 3.40 ★)
8. **Rule-based baselines:** TextBlob (F1 = 0.91) outperforms VADER (F1 = 0.79) on this synthetic corpus; results may differ on real noisy text.
9. **XGBoost** is the only ML model below perfect on synthetic data (F1 = 0.990), reflecting its sensitivity to class imbalance and token sparsity.

---

## ⚠️ Critique & Limitations
